...
- [x] 1.2 Validate `refactor-development-workflow` with strict OpenSpec validation in CI.
...
- [x] 3.2 Confirm the PR `openspec-check`, `lint`, and other existing required checks are green.
...
- [x] 3.4 Open PR to `develop` with conventional-commit title and document the follow-up order: `refactor-ci-critical-path` → `add-macos-compatibility-matrix` → `add-windows-wsl2-platform`.
