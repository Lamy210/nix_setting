...
- [x] 2.2 Keep Tauri/GTK apt dependencies only in `rust-build-smoke`; `rust-quality` must remain desktop-dependency-free.
- [x] 2.3 Make `rust-build-smoke` run CLI release smoke plus Linux desktop `cargo check`, while required `release-artifact-check` retains macOS DMG/Tauri full build coverage.
...
- [x] 2.5 Keep required coverage equivalent without duplicate Linux desktop full link/build work.
...
- [x] 3.4 Confirm GitHub branch-protection server settings are unchanged in this change.
...
- [x] 4.2 Run `actionlint` and strict OpenSpec validation through final PR CI.
...
- [x] 5.3 Record post-change two-worker/aggregator durations from a successful PR run: run #397 required critical path 327s, Rust runner total 443s.
- [x] 5.4 Evaluate target: >=25% required critical-path reduction (<=~359s) and <=20% Rust runner-time increase (<=537.6s). Result: 31.7% reduction and 443s runner total; both pass.
- [x] 5.5 If the two-worker cost/performance target is materially missed, adjust build duplication or fan-out granularity again before merge. Not required after final flake-gate redesign met both guards.
